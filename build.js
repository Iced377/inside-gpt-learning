...
// Remove the old emergency overflow-hiding patch. The production stylesheet below
// performs real mobile reflow instead of simply clipping anything outside the viewport.
...
if (!html.includes('/mobile-v2.css')) {
  html = html.replace('</head>', '<link rel="stylesheet" href="/mobile-v2.css?v=9">\n</head>');
}
if (!html.includes('/mobile-v2.js')) {
  html = html.replace('</body>', '<script src="/mobile-v2.js?v=9" defer></script>\n</body>');
}

if (html.includes('\\n')) throw new Error('Build still contains literal \\n sequences');
fs.writeFileSync(path.join(out, 'index.html'), html);

for (const file of ['mobile-v2.css','mobile-v2.js','manifest.webmanifest','icon.svg','art-1.webp','art-2.webp','art-3.webp','art-4.webp']) {
  fs.copyFileSync(path.join(root, file), path.join(out, file));
}

// Network-first navigation prevents a stale classroom page after a new production push.
const sw = `const CACHE='inside-gpt-v9';
const ASSETS=['/','/index.html','/mobile-v2.css?v=9','/mobile-v2.js?v=9','/manifest.webmanifest','/icon.svg','/art-1.webp','/art-2.webp','/art-3.webp','/art-4.webp'];
